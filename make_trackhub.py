...
def get_args():
    ...
    parser = argparse.ArgumentParser(
        ...
        epilog='Example: python get_trackhub.py --source ./bigWig --hub demo \
                --genome hg19 --server aaaaaa')
    ...
    parser.add_argument('--short-label', default=None, dest='short_label',
        help='short label for the hub, default: equal to --hub')
    parser.add_argument('--long-label', default=None, dest='long_label',
        help='long label for the hub, default: equal to --hub')
    ...
    parser.add_argument('--server', 
        help='Name of directory to save track files, default: --hub')
    parser.add_argument('--server-dir', dest='server_dir',
        default='/data/ucsc_gb/trackhub_open/',
        help='Directory to save the track files, default: /data/ucsc_gb/trackhub_open/')
...
def subgroups_picker(fn):
    ...
    fn_flag = re.fullmatch('_rep[1234]', fn_name)
    if fn_flag is None:
        rep = 0
    else:
        rep = re.sub('_rep', '', fn_flag.string) # the number
    track_subgroup = {
        'rep': rep,
        'strand': strand,
        'kind': kind,
...
def main():
    ...
    # bedgraph_files = []
    # print(len(bw_dirs))
    # print(source_checker(args.source))
    for bw_dir in bw_dirs:
        # bw = glob.glob(os.path.join(args.source, '*.bigWig'))
        # bb = glob.glob(os.path.join(args.source, '*.bigBed'))
        # bg = glob.glob(os.path.join(args.source, '*.bedGraph'))
        bw = findfiles('*.bigwig', bw_dir)
        bw.extend(findfiles('*.bw', bw_dir))
        bb = findfiles('*.bigbed', bw_dir)
        bb.extend(findfiles('*.bb', bw_dir))

        # bg = findfiles('*.bedgraph')
        # bg.extend(findfiles('*.bg'))
        bigwig_files.extend(bw)
        bigbed_files.extend(bb)
        # bedgraph_files.extend(bg) 
