...
# gene annotation from Ensembl latest version

# featureCounts -T 16 -s 1 -o $count_anti -M -O --fraction -a $gene_gtf ${bam_list[@]}
# featureCounts -T 16 -s 2 -o $count_sens -M -O --fraction -a $gene_gtf ${bam_list[@]}
...
def get_args():
    ...
    parser = argparse.ArgumentParser(
        ...
        description='Script to parse GTF file from Ensembl',
        ...
        'Modified by Ming Wang for featureCounts')
    parser.add_argument('-i', '--input', required=True, help='GTF file from Enseml')
    parser.add_argument('-o', '--output', required=True, help='output GTF file')
    parser.add_argument('-r', '--aggregate', choices=['yes', 'no'], default='yes',
        help='Indicates whether two or more genes sharing an exon should be \
              merged into an \'aggregate gene\'. default: yes')
    parser.add_argument('-f', '--featurecounts_gtf', action='store_true',
        help='save gtf file for featureCounts')
