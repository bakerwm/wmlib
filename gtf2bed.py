...
#         out = None
    name = d.get(key, None)
    if name is None:
        name = d.get('gene_id', None)
    return name


def gtf2bed(fh, feature='gene', gene_biotype=None):
    """
    feature str
...
def main():
    ...
    with xopen(gtf_in) as r, xopen(bed_out, 'wt') as w:
        for bed in gtf2bed(r, gene_biotype='protein_coding'):
            w.write(bed+'\n')
#     try:
