...
function ovl2 () {
    ...
    # nab=$(ovl $1 $2)
    # nba=$(ovl $2 $1)
    nab=$(grep "${fte}:" $2 | bedtools intersect -u -a $1 -b - | wc -l)
    nba=$(grep "${fte}:" $2 | bedtools intersect -u -a - -b $1 | wc -l)
    nte=$(grep -c "${fte}:" $2)
    # echo ${fte}
...
function ovl3 () {
    ...
    local overlap="/data/yulab/wangming/work/yu_2023/projects/20230217_zp_TE_yy209/data/te_ins/gal4_ins.overlap.bed"
    # 1. ilmn
    ovl2 $1 ${ilmn}
    ovl2 $1 ${ont}
    ovl2 $1 ${overlap}
...
function find_ins () {
    ...
    bed5_filt="${out_dir5}/${prefix}.nodup.merge.cutoff_${cutoff}.bed"
    ...
    txt="# filter by mapQ \n
        ...
        # filter by cutoff=3 \n
        awk '\$5>${cutoff}' ${bed5_merge} > ${bed5_filt}"
    echo -e ${txt} > ${cmd5}
    ...
    fig7="${stat7/.csv/.png}"
    src="/data/yulab/wangming/work/yu_2023/projects/20230217_zp_TE_yy209/script/ins_overlap.R"
    [[ ! -f ${fig7} ]] && Rscript ${src} ${stat7}
...
parallel -j 4 find_ins {} results ::: data/clean_data/*/*1.fq.gz
