...
function install_extension() {
    # install specific jupyterlab extensions
    ext_list=("@jupyterlab/toc" "@jupyterlab/github" "@jupyterlab/git" "jupyterlab-tailwind-theme" "jupyterlab-execute-time")
...
function uninstall_extension() {
    ...
    do
        echo "- ${i}"
        jupyter labextension uninstall $i
    done
}
...
# Command-line arg
[[ $# -lt 1 ]] && echo "run_jupyter.sh [start|stop|restart|install_ext|uninstall_ext]" && exit 1
...
case $1 in 
    ...
    install_ext) install_extension ;;
    uninstall_ext) uninstall_extension ;;
    *)       echo "[$1] unknown, choose [start|stop|restart|install_ext|uninstall_ext]" ;;
esac
